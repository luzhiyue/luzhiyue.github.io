;(function () {
  // 如果已经存在实例，先销毁
  if (window.minesweeperInstance) {
    const oldInstance = window.minesweeperInstance
    // 清理事件监听器
    oldInstance.startButton?.removeEventListener('click', oldInstance.startNewGame)
    oldInstance.gridElement?.removeEventListener('click', oldInstance.handleClick)
    oldInstance.gridElement?.removeEventListener('contextmenu', oldInstance.handleRightClick)
    // 清理计时器
    if (oldInstance.timerInterval) {
      clearInterval(oldInstance.timerInterval)
    }
    window.minesweeperInstance = null
  }
  ...
  // 创建新实例并保存到window对象
  window.minesweeperInstance = new Minesweeper()
})()
