;(function () {
  ...
  if (window.snakeInstance) {
    // 清理事件监听器
    document.removeEventListener('keydown', window.snakeInstance.handleKeydown)
    document.getElementById('start-btn')?.removeEventListener('click', window.snakeInstance.start)
    document
      .getElementById('pause-btn')
      ?.removeEventListener('click', window.snakeInstance.togglePause)

    // 清除游戏循环
    if (window.snakeInstance.gameInterval) {
      clearInterval(window.snakeInstance.gameInterval)
    }
  ...
  // 创建新实例并保存到window对象
  window.snakeInstance = new Snake()
})()
