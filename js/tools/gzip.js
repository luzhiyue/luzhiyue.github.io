;(function () {
  // 如果已经存在实例，先销毁
  if (window.gzipToolInstance) {
    // 清理事件监听器
    const instance = window.gzipToolInstance
    instance.inputText?.removeEventListener('input', instance.processInput)
    instance.clearBtn?.removeEventListener('click', instance.clear)
    instance.outputFormat?.removeEventListener('change', instance.processInput)
    instance.modeBtns?.forEach((btn) => {
      btn?.removeEventListener('click', instance.processInput)
    })
  ...
  class GZipTool {
    ...
    updateCounts() {
      ...
      if (inputText && outputText) {
        const ratio = (
          (1 - new Blob([outputText]).size / new Blob([inputText]).size) *
          100
        ).toFixed(2)
  ...
  // 创建新实例并保存到window对象
  window.gzipToolInstance = new GZipTool()
})()
