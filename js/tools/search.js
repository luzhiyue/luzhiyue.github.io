;(function () {
  // 如果已经存在实例，先销毁
  if (window.toolSearchInstance) {
    // 清理事件监听器
    window.toolSearchInstance.searchInput?.removeEventListener(
      'input',
      window.toolSearchInstance.handleSearch
    )
    window.toolSearchInstance.searchInput?.removeEventListener(
      'keyup',
      window.toolSearchInstance.clearSearch
    )
    window.toolSearchInstance.clearButton?.removeEventListener(
      'click',
      window.toolSearchInstance.clearSearch
    )

    window.toolSearchInstance = null
  }
  ...
  // 创建新实例并保存到window对象
  window.toolSearchInstance = new ToolSearch()
})()
