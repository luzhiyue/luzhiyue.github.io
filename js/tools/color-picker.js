;(function () {
  ...
  if (window.colorPickerInstance) {
    // 清理事件监听器
    const instance = window.colorPickerInstance
    instance.colorPicker?.removeEventListener('input', instance.updateColor)
    instance.colorInput?.removeEventListener('input', instance.updateColor)
    instance.hueSlider?.removeEventListener('input', instance.updateFromHSL)
    instance.saturationSlider?.removeEventListener('input', instance.updateFromHSL)
    instance.lightnessSlider?.removeEventListener('input', instance.updateFromHSL)
    instance.schemeButtons?.forEach((btn) => {
      btn?.removeEventListener('click', instance.generateColorScheme)
    })

    window.colorPickerInstance = null
  }
  ...
  // 创建新实例并保存到window对象
  window.colorPickerInstance = new ColorPicker()
})()
