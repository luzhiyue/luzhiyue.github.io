;(function () {
  // 如果已经存在实例，先销毁
  if (window.jsonYamlConverterInstance) {
    // 清理事件监听器
    window.jsonYamlConverterInstance.convertBtn?.removeEventListener(
      'click',
      window.jsonYamlConverterInstance.convert
    )
    window.jsonYamlConverterInstance.swapBtn?.removeEventListener(
      'click',
      window.jsonYamlConverterInstance.swap
    )
    window.jsonYamlConverterInstance.clearBtn?.removeEventListener(
      'click',
      window.jsonYamlConverterInstance.clear
    )
    window.jsonYamlConverterInstance.formatBtn?.removeEventListener(
      'click',
      window.jsonYamlConverterInstance.format
    )
    window.jsonYamlConverterInstance.copyBtn?.removeEventListener(
      'click',
      window.jsonYamlConverterInstance.copy
    )
    window.jsonYamlConverterInstance.inputText?.removeEventListener(
      'input',
      window.jsonYamlConverterInstance.convert
    )

    window.jsonYamlConverterInstance = null
  }
  ...
  // 创建新实例并保存到window对象
  window.jsonYamlConverterInstance = new JsonYamlConverter()
})()
