;(function () {
  // 如果已经存在实例，先销毁
  if (window.cronParserInstance) {
    // 清理事件监听器
    const instance = window.cronParserInstance
    instance.parseBtn?.removeEventListener('click', instance.parseCron)
    instance.exampleBtn?.removeEventListener('click', instance.showExample)
    instance.clearBtn?.removeEventListener('click', instance.clearAll)
    instance.inputCron?.removeEventListener('input', instance.parseCron)
    instance.inputCron?.removeEventListener('keypress', instance.handleKeyPress)
    ...
    window.cronParserInstance = null
  }
  ...
  // 创建新实例并保存到window对象
  window.cronParserInstance = new CronParser()
})()
